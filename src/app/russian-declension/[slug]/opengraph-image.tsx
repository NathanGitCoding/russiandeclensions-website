...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const genderLabel =
    og.genderBadges[word.gender as 'masculine' | 'feminine' | 'neuter'] ?? word.gender;
  const typeLabel = getWordPageTranslations(lang).type[word.type] ?? word.type.replace(/_/g, ' ');
      ...
      >
        ...
        {CASE_ROW_KEYS.map((key, index) => {
          const sg = (word as WordWithDeclensions)[
            `${key}_sg` as keyof WordWithDeclensions
          ] as string;
          const pl = (word as WordWithDeclensions)[
            `${key}_pl` as keyof WordWithDeclensions
          ] as string;
