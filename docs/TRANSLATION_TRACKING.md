...
**Périmètre cible (corps des leçons) :** même logique que les articles — on ne vise activement que **FR, DE, TR, PL**. Les autres langues d’UI (ES, IT, PT, NL) restent en repli **EN** sur ces pages. _Note :_ le dépôt contient encore **`ru_ru`** pour les deux leçons ci-dessous ; ce n’est pas dans le périmètre des quatre langues ci-dessus si vous standardisez le process.
